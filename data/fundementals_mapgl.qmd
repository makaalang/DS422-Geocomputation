...
sc_scale <- interpolate_palette(
  data = sc_median_income,
  column = "estimate",
  method = "equal",
  n = 4,
...
mapboxgl(bounds = sc_median_income) |>
  add_fill_layer(
    id = "sc_tracts",
    source = sc_median_income,
    fill_color = sc_scale$expression,
    fill_opacity = 0.5
  ) |>
  add_legend(
    "Median income in South Carolina",
    values = get_legend_labels(sc_scale, digits = 0, prefix = "$"),
    colors = get_legend_colors(sc_scale),
    type = "continuous"
  )
