...
library(pastecs)
stat.desc<-(penguins_data_from_csv)
```
...
ggplot(penguins_data_from_csv, aes(x = culmen_depth_mm, fill = species)) +
  ...
  labs(
    title = "Distribution of Culmen Depth by Species",
    x = "Culmen Depth (mm)",
    y = "Number of Penguins")
```
...
```{r}
ggplot(data = penguins_data_from_csv, 
       aes(x = culmen_depth_mm, y = body_mass_g, fill = species)) +
  geom_point(shape = 21, size = 3, color = "black") +
  scale_fill_viridis_d(option = "inferno") +
  theme_bw() +
  labs(
    title = "Penguin Body Mass by Culmen Depth",
    subtitle = "Colored by Penguin Species",
    x = "Culmen Depth (mm)",
    y = "Body Mass (g)",
    fill = "Species"
  )
